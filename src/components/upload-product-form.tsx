...
export default function UploadProductForm({
  ...
  const labelCls = "text-xs font-medium font-mono text-neutral-700";
  const inputCls =
    "w-full border border-black p-2 font-mono pr-16 outline-none";
  const selectCls =
    "w-full rounded-md border border-neutral-300 font-mono bg-white px-3 py-2 text-sm outline-none focus:border-neutral-900";
      ...
      <form onSubmit={onSubmit} className="grid gap-4">
        ...
        <div className="flex items-center justify-between border-t border-neutral-200 pt-4">
          <div className="text-xs">
            ...
            {status.type === "ok" && (
              <span className="text-green-700">{status.msg}</span>
            )}
            {status.type === "err" && (
              <span className="text-red-700">{status.msg}</span>
            )}
          </div>
          ...
          <button
            ...
            className="w-full font-mono rounded-sm bg-accent1 p-2 text-center font-bold text-white hover:bg-accent1"
          >
            Save Product
          </button>
        </div>
